...
extern "C" EdlibAlignResult edlibAlign(const char *const queryOriginal,
  ...
  Word *Peq =
      buildPeq((int)alphabet.size(), query, queryLength, equalityDefinition);
  /*-------------------------------------------------------*/
  ...
  if (result.editDistance >= 0) { // If there is solution.
    ...
    if (config.task == EDLIB_TASK_LOC || config.task == EDLIB_TASK_PATH) {
      ...
      if (config.mode ==
        ...
        Word *rPeq = buildPeq((int)alphabet.size(), rQuery, queryLength,
                              equalityDefinition);
    ...
    if (config.task == EDLIB_TASK_PATH) {
      ...
      obtainAlignment(query, rQuery, queryLength, alnTarget, rAlnTarget,
                      alnTargetLength, equalityDefinition, (int)alphabet.size(),
                      result.editDistance, &(result.alignment),
...
static inline int calculateBlock(Word Pv, Word Mv, Word Eq, const int hin,
  ...
  Word hinIsNeg =
      (Word)(hin >> 2) & WORD_1; // 00...001 if hin is -1, 00...000 if 0 or 1
...
static int obtainAlignmentTraceback(const int queryLength,
  ...
  *alignment = (unsigned char *)malloc((queryLength + targetLength - 1) *
  ...
  *alignment = (unsigned char *)realloc(*alignment, (*alignmentLength) *
                                                        sizeof(unsigned char));
...
obtainAlignment(const unsigned char *const query,
  ...
  if (queryLength == 0 || targetLength == 0) {
    ...
    *alignment =
        (unsigned char *)malloc((*alignmentLength) * sizeof(unsigned char));
    for (int i = 0; i < *alignmentLength; i++) {
      (*alignment)[i] =
  ...
  long long alignmentDataSize = (long long)(2 * sizeof(Word) + sizeof(int)) *
                                    maxNumBlocks * targetLength +
...
static int obtainAlignmentHirschberg(
  ...
  int queryIdxLeftAlignment; // Query/row index of cell in left column where
...
static string transformSequences(const char *const queryOriginal,
  ...
  *queryTransformed =
      (unsigned char *)malloc(sizeof(unsigned char) * queryLength);
  *targetTransformed =
      (unsigned char *)malloc(sizeof(unsigned char) * targetLength);
